...
### Week 2 — BLOCKER, do this first
- [ ] **The test suite hangs, so the last two commits are unverified.** `tests/week2.test.js` passes every assertion and then never exits: `phase0` runs in 5s and `week1` in 6s, but `week2` sits for minutes and the runner eventually reports `[error] request failed`. Cause: the `freeze(iso, fn)` helper swaps the **global** `Date` for the duration of its callback, and several tests await a real HTTP round-trip or a snapshot run inside it. Node's `fetch` keeps its sockets alive on a clock that then never advances, so the connection never times out, `server.close()` in the file's `finally` never calls back, and the process hangs holding a listening server and a keep-alive socket pair (confirmed with a `process._getActiveHandles()` probe). Removing the freeze around one page fetch moved the hang from the footer test to the next one that freezes across `await`, which confirms the mechanism. **The fix is to stop freezing global `Date` across async work**: freeze only synchronous calls, and for anything that needs a fixed "now" server-side, pass the date in — `utilisationYtd(kind, today)`, `reg12Last12Months(ownaId, today)` and friends already take one, and `services/calendar.js today()` is the single source the rest can be threaded through. Until this is fixed nothing in Week 2 can be honestly called verified, and CI would hang too
