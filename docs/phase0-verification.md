...
- API response bodies, AI provider messages and general exception details are withheld from errors/logging paths changed here. Authenticated responses use no-store, no-referrer, nosniff and frame-denial headers. Cross-origin writes with an Origin header are rejected.
